...
export default function AviaryJournal(){
  ...
  return <main className="journal" id="top">
    ...
    <header className="jhero"><div className="heroCopy"><p className="kicker">A PERSONAL RECORD OF LOOKING UP</p><h1>Birds I’ve met<br/><em>along the way.</em></h1><p className="lede">A living field journal grown from two years of listening, wandering, and paying attention.</p><a className="enter" href="#field-guide">Enter the field guide <span>↓</span></a></div><div className="heroHabitat" aria-hidden="true"><div className="orb"/><div className="flightBird bigBird"><i/></div><div className="flightBird smallBird"><i/></div><div className="reed r1"/><div className="reed r2"/><div className="reed r3"/></div><div className="stats"><span><b>51</b><small>encounters</small></span><span><b>28</b><small>species</small></span><span><b>6</b><small>regions</small></span><span><b>2</b><small>years listening</small></span></div></header>
    ...
    <section className="journeys" id="journeys"><div className="journeyIntro"><p className="kicker">WHERE THE LIST GREW</p><h2>Six regions.<br/><em>One long flight.</em></h2><p>The coordinates stay private. The shape of the journey remains.</p></div><div className="journeyList">{journeys.map((j,i)=><div className={`journeyRow ${j.tone}`} key={j.name}><span>{String(i+1).padStart(2,"0")}</span><b>{j.name}</b><div className="bar"><i style={{width:`${Math.max(8,j.count/31*100)}%`}}/></div><strong>{j.count}</strong><small>{j.count===1?"moment":"moments"}</small></div>)}</div></section>
    <section className="closing"><p className="kicker">THE FIRST ENTRY · MARCH 27, 2024</p><blockquote>“The more I listen,<br/>the more the world appears.”</blockquote><p>Last updated from Merlin / eBird · August 18, 2026</p></section>
    <footer><a href="#top" className="jbrand"><i>✦</i> SALENE'S AVIARY</a><p>Made from real encounters. Exact locations kept private.</p><span>51 moments & counting</span></footer>
