...
export default function AviaryJournal(){
  ...
  return <main className="journal" id="top">
    <nav className="jnav"><a href="#top" className="jbrand"><i>✦</i> SALENE'S AVIARY</a><div><a href="#field-guide">Field guide</a><a href="#journeys">Journeys</a></div></nav>
    <header className="jhero"><div className="heroCopy"><h1>Birds I’ve met<br/><em>along the way.</em></h1><a className="enter" href="#field-guide">Field guide <span>↓</span></a></div><div className="heroHabitat" aria-hidden="true"><div className="orb"/><div className="flightBird bigBird"><i/></div><div className="flightBird smallBird"><i/></div><div className="reed r1"/><div className="reed r2"/><div className="reed r3"/></div><div className="stats"><span><b>51</b><small>encounters</small></span><span><b>28</b><small>species</small></span><span><b>6</b><small>regions</small></span><span><b>2024—26</b><small>years</small></span></div></header>
    <section className="guide" id="field-guide"><div className="sectionTop"><div><h2>Field guide</h2></div></div><div className="toolbar"><label className="search"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search" aria-label="Search birds"/></label><div className="regionFilters">{["All","California","China","Elsewhere"].map(r=><button key={r} className={region===r?"active":""} onClick={()=>setRegion(r)}>{r}</button>)}</div><select value={sort} onChange={e=>setSort(e.target.value as typeof sort)} aria-label="Sort birds"><option value="recent">Recent</option><option value="encounters">Most seen</option><option value="name">A–Z</option></select></div><p className="resultCount">{filtered.length} {filtered.length===1?"bird":"birds"}</p><div className="speciesGrid">{filtered.map(b=><button className="speciesCard" key={b.name} onClick={()=>setSelected(b)}><div className={`speciesArt hue${(birds.indexOf(b)%6)+1}`}><span className="index">{String(birds.indexOf(b)+1).padStart(2,"0")}</span><div className="miniBird"><i/></div><span className="rings"/></div><div className="speciesCopy"><small>{b.encounters}×</small><h3>{b.name}</h3><em>{b.scientific}</em><p>{b.regions.join(" · ")}</p></div></button>)}</div>{filtered.length===0&&<div className="empty">No matches.</div>}</section>
    <section className="journeys" id="journeys"><div className="journeyIntro"><h2>2024—2026</h2><div className="yearTrail"><span><b>2024</b><small>3</small></span><span className="quiet"><b>2025</b><small>—</small></span><span><b>2026</b><small>48</small></span></div></div><div className="journeyList">{journeys.map((j,i)=><div className={`journeyRow ${j.tone}`} key={j.name}><span>{String(i+1).padStart(2,"0")}</span><b>{j.name}</b><div className="bar"><i style={{width:`${Math.max(8,j.count/31*100)}%`}}/></div><strong>{j.count}</strong></div>)}</div></section>
    <footer><a href="#top" className="jbrand"><i>✦</i> SALENE'S AVIARY</a></footer>
    {selected&&<div className="detailShade" onMouseDown={()=>setSelected(null)}><aside className="detail" onMouseDown={e=>e.stopPropagation()} aria-modal="true" role="dialog"><button className="detailClose" onClick={()=>setSelected(null)} aria-label="Close">×</button><div className={`detailArt hue${(birds.indexOf(selected)%6)+1}`}><div className="detailBird"><i/></div></div><div className="detailBody"><p className="kicker">{selected.encounters} {selected.encounters===1?"ENCOUNTER":"ENCOUNTERS"}</p><h2>{selected.name}</h2><em>{selected.scientific}</em><dl><div><dt>First seen</dt><dd>{fmt(selected.first)}</dd></div><div><dt>Latest</dt><dd>{fmt(selected.latest)}</dd></div><div><dt>Regions</dt><dd>{selected.regions.join(", ")}</dd></div></dl></div></aside></div>}
  </main>
}
