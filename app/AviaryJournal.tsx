...
const journeys=[
  {name:"California",count:31,tone:"clay",x:13,y:47},
  {name:"Ontario",count:2,tone:"sage",x:28,y:29},
  {name:"New York",count:2,tone:"blue",x:31,y:39},
  {name:"Yunnan",count:10,tone:"moss",x:76,y:51},
  {name:"Guangdong",count:5,tone:"gold",x:82,y:58},
  {name:"Hong Kong",count:1,tone:"rose",x:86,y:62}
];
...
export default function AviaryJournal(){
  const [query,setQuery]=useState(""); const [region,setRegion]=useState("All"); const [mapPlace,setMapPlace]=useState("All"); const [sort,setSort]=useState<"recent"|"name"|"encounters">("recent"); const [selected,setSelected]=useState<Bird|null>(null);
  ...
  const mapStats=useMemo(()=>{
    if(mapPlace==="All") return {species:birds.length,encounters:journeys.reduce((sum,place)=>sum+place.count,0)};
    return {species:birds.filter(b=>b.regions.includes(mapPlace)).length,encounters:journeys.find(place=>place.name===mapPlace)?.count??0};
  },[mapPlace]);
  const explorePlace=()=>{setRegion(mapPlace);document.getElementById("field-guide")?.scrollIntoView({behavior:"smooth"})};
  return <main className="journal" id="top">
    ...
    <section className="guide" id="field-guide"><div className="sectionTop"><div><h2>Field guide</h2></div></div><div className="toolbar"><label className="search"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search the canopy" aria-label="Search birds"/></label><div className="regionFilters">{["All",...journeys.map(j=>j.name)].map(r=><button key={r} className={region===r?"active":""} onClick={()=>setRegion(r)}>{r}</button>)}</div><select value={sort} onChange={e=>setSort(e.target.value as typeof sort)} aria-label="Sort birds"><option value="recent">Recent</option><option value="encounters">Most seen</option><option value="name">A–Z</option></select></div><p className="resultCount">{filtered.length} {filtered.length===1?"species":"species"}{region!=="All"&&<> in {region}</>}</p><div className="speciesGrid">{filtered.map(b=><button className="speciesCard" key={b.name} onClick={()=>setSelected(b)}><div className={`speciesArt hue${(birds.indexOf(b)%6)+1}`}><img src={imageFor(b)} alt="" loading="lazy"/></div><div className="speciesCopy"><small>{b.encounters}×</small><h3>{b.name}</h3><em>{b.scientific}</em><p>{b.regions.join(" · ")}</p></div></button>)}</div>{filtered.length===0&&<div className="empty">No matches.</div>}</section>
    <section className="places" id="journeys"><div className="placesHead"><p className="kicker">PLACES</p><h2>Where I heard them.</h2></div><div className="mapShell"><div className="encounterMap" aria-label="Broad regions where birds were encountered"><div className="land landAmericas"/><div className="land landEurasia"/><div className="mapLine lineOne"/><div className="mapLine lineTwo"/>{journeys.map(place=><button key={place.name} className={`mapPin ${place.tone} ${mapPlace===place.name?"active":""}`} style={{left:`${place.x}%`,top:`${place.y}%`}} onClick={()=>setMapPlace(place.name)} aria-pressed={mapPlace===place.name}><i/><span>{place.name}</span></button>)}</div><aside className="placeCard" aria-live="polite"><div><button className={mapPlace==="All"?"placeAll active":"placeAll"} onClick={()=>setMapPlace("All")}>All places</button><p className="placeName">{mapPlace}</p></div><div className="placeNumbers"><span><b>{mapStats.species}</b><small>species</small></span><span><b>{mapStats.encounters}</b><small>encounters</small></span></div><button className="explorePlace" onClick={explorePlace}>See {mapStats.species} species <span>↗</span></button></aside></div></section>
